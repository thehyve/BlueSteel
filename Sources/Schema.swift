...
public enum Schema {
    ...
    private static func validate(unique: [String], context: AvroCodingContext) throws {
        let duplicates = Dictionary(grouping: unique, by: { $0 })
            .filter { $1.count > 1 }

        guard duplicates.isEmpty else {
            throw ValidationError.notUnique(duplicates.map { $1.first! }, context)
        }
    }
